...
export const addSchoolsAction: ActionFunction = async ({ request }) => {
  ...
  if (!schoolName || !emailDomain) {
    return json({ error: "School name and email domain are required." }, { status: 400 });
  }
  ...
  try {
    ...
    if (!response.ok) {
      ...
      return json({ error: errorData.error || "Failed to add school" }, { status: response.status });
    }
    ...
    return json({ error: "An unknown error occurred while adding the school" }, { status: 500 });
  }
};
